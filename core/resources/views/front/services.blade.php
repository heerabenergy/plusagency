                    ...
                    <div class="col-md-6">
                       <div class="single-service">
                          <div class="service-img-wrapper">
                             <img class="lazy" data-src="{{asset('assets/front/img/services/'.$service->main_image)}}" alt="{{__($service->slug)}}">
                          </div>
                          <div class="service-txt">

                            <h4 class="service-title"><a aria-label="{{$service->title}}" @if($service->details_page_status == 1) href="{{route('front.servicedetails', [$service->slug])}}" @endif>{{convertUtf8(strlen($service->title)) > 18 ? convertUtf8(substr($service->title, 0, 18)) . '...' : convertUtf8($service->title)}}</a></h4>
                            ...
                            <p class="service-summary">
                             ...
                             <a aria-label="{{__('Read More')}}" href="{{route('front.servicedetails', [$service->slug])}}" class="readmore-btn"><span>{{__('Read More')}}</span></a>
                             @endif
                ...
                <div class="category-lists job">
                   ...
                   <ul>
                     @foreach ($scats as $key => $scat)
                       <li class="single-category {{$scat->id == request()->input('category') ? 'active' : ''}}"><a aria-label="{{$scat->name}}" href="{{route('front.services', ['category' => $scat->id, 'term'=>request()->input('term')])}}">{{convertUtf8($scat->name)}}</a></li>
                     @endforeach
                   </ul>
                </div>
             </div>
