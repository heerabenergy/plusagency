<div class="nav-menu d-flex justify-content-end align-items-center">
    ...
    <nav class="main-menu">
        <ul>
            ...
            @foreach ($links as $link)
                ...
                @else
                    @if (!array_key_exists("children",$link))
                        ...
                        <li class="menu-item"><a aria-label="{{__($link['text'])}}" href="{{$href}}" target="{{$link["target"]}}">{{$link["text"]}}</a></li>
                    @else
            ...
            @if ($bs->is_quote == 1)
            <li>
                <a aria-label="{{__('Request A Quote')}}" href="{{route('front.quote')}}" class="quote-btn">{{__('Request A Quote')}}</a>
            </li>
            @endif
        </ul>
    </nav>
