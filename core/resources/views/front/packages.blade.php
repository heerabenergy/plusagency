              ...
              <div class="single-pricing-table">
                  ...
                  @auth
                    @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                      @if ($activeSub->first()->current_package_id == $package->id)
                        <a aria-label="{{__('Extend')}}" href="{{route('front.packageorder.index',$package->id)}}" class="pricing-btn">{{__('Extend')}}</a>
                      @else
                        <a aria-label="{{__('Change')}}" href="{{route('front.packageorder.index',$package->id)}}" class="pricing-btn">{{__('Change')}}</a>
                      @endif
                    @elseif ($activeSub->count() == 0)
                      <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="pricing-btn">{{__('Purchase')}}</a>
                    @endif
                  @endauth
                ...
                @else
                  @if ($package->order_status != 0)
                    ...
                    <a aria-label="{{__('Place Order')}}" href="{{ $link }}" @if($package->order_status == 2) target="_blank" @endif class="pricing-btn">{{__('Place Order')}}</a>
                  @endif
                @endif
              </div>
            </div>
