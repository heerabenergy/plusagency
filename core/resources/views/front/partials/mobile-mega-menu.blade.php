<li class="menu-item menu-item-has-children static mega-dropdown d-lg-none d-block"><a aria-label="{{__($link["text"])}}" href="{{$href}}">{{$link["text"]}}</a>
    <ul class="mega-menu">
        <li class="mega-wrap">
                @if ($catAvailable)

                    <a aria-label="{{__('All')}}" href="{{$allUrl}}" data-tabid="all">{{__('All')}}</a>
                    @foreach ($megaMenus as $mCatId => $mItemIds)
                        ...
                        @endphp
                        <a aria-label="{{__($mcat->name)}}" href="{{$catUrl}}" data-tabid="#megaTab{{$link["type"]}}{{$mcat->id}}">{{$mcat->name}}</a>
                    @endforeach
                ...
                @endphp
                    @if (!$catAvailable)
                        @foreach ($megaMenus as $mItemId)
                            ...
                            @endphp
                            <a aria-label="{{__($mItem->title)}}" href="{{$detailsUrl}}">{{strlen($mItem->title) > 30 ? mb_substr($mItem->title,0,30,'utf-8') . '...' : $mItem->title}}</a>
