      ...
      @if ($errors->any())
        @foreach ($errors->all() as $error)
          <div class="alert alert-danger alert-block">
            <strong>{{ $error }}</strong>
            <button aria-label="{{__('Close')}}" type="button" class="close" data-dismiss="alert">×</button>
          </div>
        @endforeach
      @endif
      <div class="row">
        <div class="col-lg-6">
          <div class="courses-img-box">
            <img data-src="{{ asset('assets/front/img/courses/' . $course_details->course_image) }}"
              class="lazy img-fluid" alt="{{__($course_details->title)}}">
            <div class="video-box">
              <a aria-label="{{__('Play')}}" href="{{ $course_details->video_link }}" class="video-popup"><i class="fas fa-play"></i></a>
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="course-content">
            ...
            <form method="POST" id="paymentGatewayForm" enctype="multipart/form-data">
              ...
              <div class="d-flex flex-row">
                ...
                @auth
                  ...
                  @if (Auth::check() && $pcourse->count() > 0)
                    ...
                    <a aria-label="{{__('Go to this Course')}}" class="text-primary"
                    ...
                    <div
                      ...
                      <select aria-label="{{__('Pay Via')}}" name="gateway" id="paymentType" class="select-payment">
                        ...
                        @foreach ($offlineGateways as $offlineGateway)
                          <option value="{{ $offlineGateway->id }}" data-type="offline">{{ $offlineGateway->name }}
                          </option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group button mb-20">
                      <button aria-label="{{__('Enroll Now')}}" type="submit" id="enrollBtn"
                        class="main-btn {{ $course_details->current_price == null ? 'd-none' : '' }}">{{ __('Enroll Now') }}</button>
                    </div>
                    <div class="form-group button mb-20">
                      <button aria-label="{{__('Enroll Now')}}" type="submit" id="enrollBtn__free"
                        class="main-btn {{ $course_details->current_price != null ? 'd-none' : '' }}">{{ __('Enroll Now') }}</button>
                    </div>
                  @endif
                @endauth
              ...
              <div class="row mt-3 d-none" id="iyzicoTab">
                <div class="col-md-6 mb-3">
            ...
            <ul class="social-link">
              <li><a aria-label="{{__('Facebook')}}" href="#"><i class="fab fa-facebook-f"></i></a></li>
              <li><a aria-label="{{__('Twitter')}}" href="#"><i class="fab fa-twitter"></i></a></li>
              <li><a aria-label="{{__('Pinterest')}}" href="#"><i class="fab fa-pinterest-p"></i></a></li>
              <li><a aria-label="{{__('Linkedin')}}" href="#"><i class="fab fa-linkedin-in"></i></a></li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div class="discription-area">
            <div class="discription-tabs">
              <ul class="nav nav-tabs">
                <li class="nav-item">
                  <a aria-label="{{__('Overview')}}" class="nav-link active" data-toggle="tab" href="#overview">{{ __('Overview') }}</a>
                </li>
                <li class="nav-item">
                  <a aria-label="{{__('Outline')}}" class="nav-link" data-toggle="tab" href="#outline">{{ __('Outline') }}</a>
                </li>
                <li class="nav-item">
                  <a aria-label="{{__('Instructor')}}" class="nav-link" data-toggle="tab" href="#instructor">{{ __('Instructor') }}</a>
                </li>

                @if ($bex->is_course_rating == 1)
                  <li class="nav-item">
                    <a aria-label="{{__('Reviews')}}" class="nav-link" data-toggle="tab" href="#reviews">{{ __('Reviews') }}</a>
                  </li>
                @endif
              </ul>
            </div>
                ...
                <div class="content-box">
                  <div class="accordion" id="accordionExample">
                    @foreach ($modules as $module)
                      <div class="card">
                        <a aria-label="{{__('Collapse')}}" class="card-header collapsed" href="#" id="headingone" data-toggle="collapse"
                        ...
                        <div id="{{ 'collapse' . $module->id }}"
                          class="{{ $loop->first ? 'collapse show' : 'collapse' }}" aria-labelledby="headingOne"
                          ...
                          <div class="card-body">
                            ...
                            <ul>
                              @foreach ($lessons as $lesson)
                                <li><a aria-label="{{__('Play')}}"><i class="fas fa-play"></i> {{ $lesson->name }} <span
                ...
                <div class="instructor-wrap">
                  <div class="thumb">
                    <img data-src="{{ asset('assets/front/img/instructors/' . $course_details->instructor_image) }}"
                      class="img-fluid lazy" alt="{{__($course_details->instructor_name)}}">
                  </div>
                  <div class="content">
                    ...
                    <ul class="social-link">
                      @if (!empty($course_details->instructor_facebook))
                        <li><a aria-label="{{__('Facebook')}}" href="{{ $course_details->instructor_facebook }}" target="_blank"><i
                              class="fab fa-facebook-f"></i></a></li>
                      @endif
                      @if (!empty($course_details->instructor_twitter))
                        <li><a aria-label="{{__('Twitter')}}" href="{{ $course_details->instructor_twitter }}" target="_blank"><i
                              class="fab fa-twitter"></i></a></li>
                      @endif
                      @if (!empty($course_details->instructor_instagram))
                        <li><a aria-label="{{__('Instagram')}}" href="{{ $course_details->instructor_instagram }}" target="_blank"><i
                              class="fab fa-instagram"></i></a></li>
                      @endif
                      @if (!empty($course_details->instructor_linkedin))
                        <li><a aria-label="{{__('Linkedin')}}" href="{{ $course_details->instructor_linkedin }}" target="_blank"><i
                          ...
                          <div class="review_user">
                            @if (strpos($review->reviewByUser->photo, 'facebook') !== false || strpos($review->reviewByUser->photo, 'google'))
                              <img class="lazy"
                                ...
                                alt="{{__($review->reviewByUser->fname . ' ' . $review->reviewByUser->lname)}}">
                            @else
                              <img class="lazy"
                      ...
                      <div class="review_form pt-4">
                        <form action="{{ route('course.review') }}" method="POST">
                          ...
                          <div class="form_group">
                            ...
                            <div class="review-content mt-2">
                              <ul class="review-value review-1">
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="1"><i class="far fa-star"></i></a>
                                </li>
                              </ul>
                              <ul class="review-value review-2">
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="2"><i class="far fa-star"></i></a>
                                </li>
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="2"><i class="far fa-star"></i></a>
                                </li>
                              ...
                              <ul class="review-value review-3">
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="3"><i class="far fa-star"></i></a>
                                </li>
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="3"><i class="far fa-star"></i></a>
                                </li>
                              ...
                              <ul class="review-value review-4">
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="4"><i class="far fa-star"></i></a>
                                </li>
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="4"><i class="far fa-star"></i></a>
                                </li>
                              ...
                              <ul class="review-value review-5">
                                <li>
                                  <a aria-label="{{__('Rating')}}" class="cursor-pointer" data-ratingVal="5"><i class="far fa-star"></i></a>
                                </li>
                                <li>
                                  <a aria-label="{{__('Rating')}}"  class="cursor-pointer" data-ratingVal="5"><i class="far fa-star"></i></a>
                                </li>
                          ...
                          <div class="form_button">
                            <button aria-label="{{__('Submit')}}" type="submit" class="main-btn mt-2">{{ __('Submit') }}</button>
                          </div>
                        </form>
                      </div>
                    @endif
...
@section('scripts')
  ...
  <script src="{{ asset('assets/front/js/course-checkout-stripe.js') }}"></script>
  <script>
    $(document).ready(function() {
      ...
      $(document).on('click', '#enrollBtn', function() {
        ...
        if (type == 'offline') {
          $('#paymentGatewayForm').attr('action', "{{ url('/') }}" + "/course/offline/" + paymentGateway +
            "/submit");
        } else {
          ...
          } else if (paymentGateway == 'yoco') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.yoco') }}");
          } else if (paymentGateway == 'perfect_money') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.perfect_money') }}");
          } else if (paymentGateway == 'xendit') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.xendit') }}");
          } else if (paymentGateway == 'toyyibpay') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.toyyibpay') }}");
          } else if (paymentGateway == 'paytabs') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.paytabs') }}");
          } else if (paymentGateway == 'midtrans') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.midtrans') }}");
          } else if (paymentGateway == 'iyzico') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.iyzico') }}");
          } else if (paymentGateway == 'phonepe') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.phonepe') }}");
          } else if (paymentGateway == 'myfatoorah') {
            $('#paymentGatewayForm').attr('action', "{{ route('course.payment.myfatoorah') }}");
          }
        }
      ...
      $(document).on('change', '#paymentType', function() {
        ...
        if (gatewayName == 'iyzico') {
          $('#iyzicoTab').removeClass('d-none');
        } else {
          $('#iyzicoTab').addClass('d-none');
        }
