                  ...
                  <div class="col-lg-6 {{$rtl == 1 ? 'text-left' : 'text-right'}}">
                     <ul class="social-links">
                       @foreach ($socials as $key => $social)
                         <li><a aria-label="{{__('Social Icon')}}" target="_blank" href="{{$social->url}}"><i class="{{$social->icon}}"></i></a></li>
                       @endforeach
                     </ul>

                     @if (!empty($currentLang) && count($langs) > 1)
                       <div class="language">
                          <a aria-label="{{__('Toggle Language')}}" class="language-btn" href="#"><i class="flaticon-worldwide"></i> {{convertUtf8($currentLang->name)}}</a>
                          <ul class="language-dropdown">
                            @foreach ($langs as $key => $lang)
                            <li><a aria-label="{{__('Change language to')}} {{convertUtf8($lang->name)}}" href='{{ route('changeLanguage', $lang->code) }}'>{{convertUtf8($lang->name)}}</a></li>
                            @endforeach
                          </ul>
                       </div>
                     @endif
                     ...
                     @guest
                        @if ($bex->is_user_panel == 1)
                            <ul class="login">
                                <li><a aria-label="{{__('User Login')}}" href="{{route('user.login')}}">{{__('Login')}}</a></li>
                            </ul>
                        @endif
                    @endguest
                     @auth
                     <div class="language dashboard">
                         <a aria-label="{{__('Toggle Language')}}" class="language-btn" href="#">
                             <i class="far fa-user"></i> {{Auth::user()->username}}
                         </a>
                         <ul class="language-dropdown">
                             <li>
                                 <a aria-label="{{__('User Dashboard')}}" href="{{route('user-dashboard')}}">{{__('Dashboard')}}</a>
                             </li>

                             @if ($bex->recurring_billing == 1)
                                 <li><a aria-label="{{__('User Packages')}}" href="{{route('user-packages')}}">{{__('Packages')}}</a></li>
                             @endif

                             @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
                                 <li><a aria-label="{{__('Product Orders')}}" href="{{route('user-orders')}}">{{__('Product Orders')}} </a></li>
                             @endif

                             @if ($bex->recurring_billing == 0)
                                 <li><a aria-label="{{__('Package Orders')}}" href="{{route('user-package-orders')}}">{{__('Package Orders')}} </a></li>
                             @endif


                             @if ($bex->is_course == 1)
                             <li>
                                 <a aria-label="{{__('User Courses')}}" href="{{route('user.course_orders')}}" >{{__('Courses')}}</a>
                             </li>
                             @endif
                             ...
                             @if ($bex->is_event == 1)
                             <li>
                                 <a aria-label="{{__('Event Bookings')}}" href="{{route('user-events')}}" >{{__('Event Bookings')}}</a>
                             </li>
                             @endif
                             ...
                             <li>
                                 <a aria-label="{{__('User Donations')}}" href="{{route('user-donations')}}" >{{__('Donations')}}</a>
                             </li>
                             @endif

                             @if ($bex->is_ticket == 1)
                             <li>
                                 <a aria-label="{{__('Support Tickets')}}" href="{{route('user-tickets')}}">{{__('Support Tickets')}}</a>
                             </li>
                             @endif

                             <li>
                                 <a aria-label="{{__('Edit Profile')}}" href="{{route('user-profile')}}">{{__('Edit Profile')}}</a>
                             </li>

                             @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
                                 <li>
                                     <a aria-label="{{__('Shipping Details')}}" href="{{route('shpping-details')}}">{{__('Shipping Details')}}</a>
                                 </li>
                                 <li>
                                     <a aria-label="{{__('Billing Details')}}" href="{{route('billing-details')}}">{{__('Billing Details')}}</a>
                                 </li>
                                 <li>
                                     <a aria-label="{{__('Change Password')}}" href="{{route('user-reset')}}">{{__('Change Password')}}</a>
                                 </li>
                             @endif
                             <li>
                                 <a aria-label="{{__('Logout')}}" href="{{route('user-logout')}}" target="_self">{{__('Logout')}}</a>
                             </li>
                         </ul>
                     </div>
                     @endauth
               ...
               <div class="row">
                  <div class="col-lg-4 col-md-12">
                     <div class="footer-logo-wrapper">
                        <a aria-label="{{__('Home')}}" href="{{route('front.index')}}">
                        <img class="lazy" data-src="{{asset('assets/front/img/'.$bs->footer_logo)}}" alt="{{__('Footer logo')}}">
                        </a>
                     </div>
                     <p class="footer-txt">
                  ...
                  <div class="col-lg-2 col-md-3">
                     ...
                     <ul class="footer-links">
                        @foreach ($ulinks as $key => $ulink)
                          <li><a aria-label="{{convertUtf8($ulink->name)}}" href="{{$ulink->url}}">{{convertUtf8($ulink->name)}}</a></li>
                        @endforeach
                     </ul>
                  </div>
      ...
      <div id="WAButton"></div>
        ...
        @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
            <div id="cartIconWrapper">
                <a aria-label="{{__('Cart')}}" class="d-block" id="cartIcon" href="{{route('front.cart.index')}}">
