            ...
            <div class="row align-items-center">
                <div class="col-lg-7">
                    <div class="lawyer_box_img">
                        ...
                        <div class="lawyer_img">
                            <img data-src="{{asset('assets/front/img/'.$be->intro_bg2)}}" class="img-fluid lazy" alt="{{__('Intro background')}}">
                            <div class="play_box">
                ...
                <div class="col-lg-5">
                    <div class="lawyer_content_box">
                        ...
                        <div class="button_box">
                            <a aria-label="{{__($bs->intro_section_button_text)}}" href="{{$bs->intro_section_button_url}}" class="lawyer_btn">{{convertUtf8($bs->intro_section_button_text)}}</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                @if (serviceCategory())
                    @foreach ($scategories as $key => $scategory)
                        <div class="grid_item">
                            <div class="grid_inner_item">
                                <div class="lawyer_img">
                                    <img data-src="{{asset('assets/front/img/service_category_icons/'.$scategory->image)}}" class="img-fluid lazy" alt="{{__($scategory->name)}}">
                                </div>
                                <div class="lawyer_content">
                                    ...
                                    <a aria-label="{{__('View Services')}}" href="{{route('front.services', ['category'=>$scategory->id])}}" class="lawyer_btn">{{__('View Services')}}</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
                                ...
                                <div class="lawyer_img">
                                    @if (!empty($service->main_image))
                                        <div class="logistics_icon">
                                            <img data-src="{{asset('assets/front/img/services/' . $service->main_image)}}" class="img-fluid lazy" alt="{{__($service->title)}}">
                                        </div>
                                    @endif
                                </div>
                                <div class="lawyer_content">
                                    ...
                                    @if($service->details_page_status == 1)
                                        <a aria-label="{{__('Read More')}}" href="{{route('front.servicedetails', [$service->slug, $service->id])}}" class="lawyer_btn">{{__('Read More')}}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="lawyer_content_box">
                        ...
                        @if (!empty($bs->approach_button_url) && !empty($bs->approach_button_text))
                            <div class="button_box  wow fadeInUp" data-wow-delay=".4s">
                                <a aria-label="{{__($bs->approach_button_text)}}" href="{{$bs->approach_button_url}}" class="lawyer_btn">{{convertUtf8($bs->approach_button_text)}}</a>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="testimonial_slide">
                @foreach ($testimonials as $key => $testimonial)
                    <div class="testimonial_box">
                        <div class="lawyer_content_box">
                            <img data-src="{{asset('assets/front/img/quote_1.png')}}" class="lazy" alt="{{__('Testimonial icon')}}">
                        ...
                        <div class="grid_inner_item">
                            <div class="lawyer_img">
                                ...
                                <div class="overlay_content">
                                    <h3><a aria-label="{{__($portfolio->title)}}" href="{{route('front.portfoliodetails', [$portfolio->slug, $portfolio->id])}}">{{strlen($portfolio->title) > 25 ? mb_substr($portfolio->title, 0, 25, 'utf-8') . '...' : $portfolio->title}}</a></h3>
                                    @if (!empty($portfolio->service))
                                        <p>{{$portfolio->service->title}}</p>
                                    @endif
                                    <a aria-label="{{__('View More')}}" href="{{route('front.portfoliodetails', [$portfolio->slug, $portfolio->id])}}" class="lawyer_btn">{{__('View More')}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pricing_box text-center">
                        <div class="pricing_title">
                            @if (!empty($package->image))
                                <img data-src="{{asset('assets/front/img/packages/' . $package->image)}}" class="lazy" alt="{{__('Package icon')}}">
                            @endif
                        ...
                        <div class="pricing_button">
                            @if ($package->order_status == 1)
                                <a aria-label="{{__('Place Order')}}" href="{{route('front.packageorder.index', $package->id)}}" class="lawyer_btn">{{__('Place Order')}}</a>
                            @endif
                        </div>
                    </div>
                @endforeach
                            ...
                            <div class="lawyer_content text-center">
                                ...
                                <ul class="social">
                                    @if (!empty($member->facebook))
                                        <li><a aria-label="{{__($member->facebook)}}" href="{{$member->facebook}}" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                                    @endif
                                    @if (!empty($member->twitter))
                                        <li><a aria-label="{{__($member->twitter)}}" href="{{$member->twitter}}" target="_blank"><i class="fab fa-twitter"></i></a></li>
                                    @endif
                                    @if (!empty($member->linkedin))
                                        <li><a aria-label="{{__($member->linkedin)}}" href="{{$member->linkedin}}" target="_blank"><i class="fab fa-linkedin-in"></i></a></li>
                                    @endif
                                    @if (!empty($member->instagram))
                                        <li><a aria-label="{{__($member->instagram)}}" href="{{$member->instagram}}" target="_blank"><i class="fab fa-instagram"></i></a></li>
                                    @endif
                                </ul>
                            </div>
                        </div>
    ...
    <section class="lawyer_cta cta_v1 bg_image pt-70 pb-70" style="background-image: url('{{asset('assets/front/img/'.$bs->cta_bg)}}'); background-size:cover;">
        ...
        <div class="container">
            <div class="row align-items-center">
                ...
                <div class="col-lg-5">
                    <div class="button_box">
                        <a aria-label="{{__($bs->cta_section_button_text)}}" href="{{$bs->cta_section_button_url}}" class="lawyer_btn">{{convertUtf8($bs->cta_section_button_text)}}</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6">
                    <div class="button_box float-lg-right">
                        <a aria-label="{{__('View More')}}" href="{{route('front.blogs')}}" class="btn_link">{{__('View More')}}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="partner_slide">
                @foreach ($partners as $key => $partner)
                    <div class="single_partner">
                        <a aria-label="{{__($partner->name)}}" href="{{$partner->url}}"><img data-src="{{asset('assets/front/img/partners/'.$partner->image)}}" class="img-fluid lazy" alt="Partner Logo"></a>
                    </div>
                @endforeach
            </div>
        </div>
