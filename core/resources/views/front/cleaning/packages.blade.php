...
<section class="pricing-tables pt-90 pb-30" id="masonry-package">
  <div class="container">
        ...
        <div class="row">
          ...
          @else
            @foreach ($packages as $key => $package)
              ...
              <div class="col-lg-4 col-md-6 mb-5 package-column {{ $categoryName }}">
                <div class="single-price-item text-center">
                  ...
                  <h1 class="bg-1" style="background: #{{$package->color}};">
                    ...
                    @auth
                      @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                        @if ($activeSub->first()->current_package_id == $package->id)
                          <a aria-label="{{__('Extend')}}" href="{{route('front.packageorder.index',$package->id)}}" class="main-btn price-btn">{{__('Extend')}}</a>
                        @else
                          <a aria-label="{{__('Change')}}" href="{{route('front.packageorder.index',$package->id)}}" class="main-btn price-btn">{{__('Change')}}</a>
                        @endif
                      @elseif ($activeSub->count() == 0)
                        <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="main-btn price-btn">{{__('Purchase')}}</a>
                      @endif
                    @endauth
                    @guest
                      <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="main-btn price-btn">{{__('Purchase')}}</a>
                  ...
                  @else
                    @if ($package->order_status != 0)
                      ...
                      <div class="pricing_button">
                        <a aria-label="{{__('Place Order')}}" href="{{ $link }}" @if($package->order_status == 2) target="_blank" @endif class="main-btn price-btn">{{__('Place Order')}}</a>
                      </div>
                    @endif
                  @endif
                </div>
