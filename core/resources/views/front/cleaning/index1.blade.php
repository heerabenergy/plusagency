                            ...
                            @if (!empty($scat->image))
                                <div class="single-service-bg">
                                    ...
                                    <div class="single-service-link">
                                        <a aria-label="{{__('View Services')}}" href="{{route('front.services', ['category'=>$scat->id])}}" class="main-btn service-btn">{{__('View Services')}}</a>
                                    </div>
                                </div>
                            ...
                            @if (!empty($service->main_image))
                                <div class="single-service-bg">
                                    ...
                                    @if($service->details_page_status == 1)
                                        <div class="single-service-link">
                                            <a aria-label="{{__('View More')}}" href="{{route('front.servicedetails', [$service->slug, $service->id])}}" class="main-btn service-btn">{{__('View More')}}</a>
                                        </div>
                                    @endif
                                </div>
                                <div class="single-service-content">
                ...
                <div class="col-lg-6">
                    ...
                    @if (!empty($bs->approach_button_url) && !empty($bs->approach_button_text))
                        <div class="button_box">
                            <a aria-label="{{convertUtf8($bs->approach_button_text)}}" href="{{$bs->approach_button_url}}" class="main-btn about-btn">{{convertUtf8($bs->approach_button_text)}}</a>
                        </div>
                    @endif
                </div>
                <div class="col-lg-6">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="video-content" style="background-image: url('{{asset('assets/front/img/'.$bs->intro_bg)}}');background-size:cover;">
                        <a aria-label="{{__('Play Video')}}" href="{{$bs->intro_section_video_link}}" class="play-btn video-popup"><i class="fa fa-play"></i></a>
                    </div>
                    @if (!empty($bs->intro_section_button_url) && !empty($bs->intro_section_button_text))
                        <div class="video-btn-area">
                            <a aria-label="{{convertUtf8($bs->intro_section_button_text)}}" href="{{$bs->intro_section_button_url}}" class="main-btn video-btn" target="_blank">{{convertUtf8($bs->intro_section_button_text)}}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-4 text-right">
                    <a aria-label="{{__('View More')}}" href="{{route('front.portfolios')}}" class="project-btn">{{__('View More')}} <i class="fa fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
            ...
            <div class="project-slider-active project-slick">
                @foreach ($portfolios as $key => $portfolio)
                    <div class="single-project-item">
                        <img data-src="{{asset('assets/front/img/portfolios/featured/'.$portfolio->featured_image)}}" class="lazy" alt="{{__($portfolio->title)}}">
                        <div class="project-link text-center">
                            ...
                            @endif
                            <a aria-label="{{__('View Details')}}" href="{{route('front.portfoliodetails', [$portfolio->slug, $portfolio->id])}}" class="main-btn project-link-btn">{{__('View Details')}}</a>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="team-carousel-active team-slick">
                @foreach ($members as $key => $member)
                    <div class="single-team-item">
                        <img data-src="{{asset('assets/front/img/members/'.$member->image)}}" class="lazy" alt="{{__($member->name)}}">
        ...
        <div class="container">
            <div class="row align-items-center">
                ...
                <div class="col-lg-4 text-center">
                    <a aria-label="{{convertUtf8($bs->cta_section_button_text)}}" href="{{$bs->cta_section_button_url}}" class="main-btn cta-btn">{{convertUtf8($bs->cta_section_button_text)}}</a>
                </div>
            </div>
        </div>
    </section>
                ...
                @foreach ($packages as $key => $package)
                    <div class="single-price-item text-center">
                        ...
                        @if ($package->order_status == 1)
                            <a aria-label="{{__('Place Order')}}" href="{{route('front.packageorder.index', $package->id)}}" class="main-btn price-btn">{{__('Place Order')}}</a>
                        @endif
                    </div>
                @endforeach
            </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-5 text-right">
                    <a aria-label="{{__('View More')}}" href="{{route('front.blogs')}}" class="blog-link">{{__('View More')}} <i class="fa fa-arrow-right"></i></a>
                </div>
            </div>
                ...
                @foreach ($blogs as $key => $blog)
                    ...
                    <div class="single-blog-item">
                        <div class="single-blog-img">
                            <img data-src="{{asset('assets/front/img/blogs/'.$blog->main_image)}}" class="lazy" alt="Blog Image">
                        </div>
                        <div class="single-blog-details">
                            ...
                            <a aria-label="{{__('Read More')}}" href="{{route('front.blogdetails', [$blog->slug, $blog->id])}}" class="blog-btn">{{__('Read More')}} <i class="fa fa-arrow-right"></i></a>
                        </div>
                    </div>
                @endforeach
            </div>
                    ...
                    <div class="brand-container brand-carousel-active">
                        @foreach ($partners as $key => $partner)
                            <div class="single-brand-logo">
                                <a aria-label="{{__('Partner Logo')}}" class="d-block" href="{{$partner->url}}" target="_blank"><img data-src="{{asset('assets/front/img/partners/'.$partner->image)}}" class="img-fluid lazy" alt="Partner Logo"></a>
                            </div>
                        @endforeach
                    </div>
                </div>
