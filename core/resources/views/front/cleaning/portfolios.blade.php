          ...
          @foreach ($portfolios as $key => $portfolio)
            ...
            <div class="col-lg-6 mb-30 portfolio-column {{ $categoryName }}">
              <div class="single-project-item mx-0">
                <img src="{{asset('assets/front/img/portfolios/featured/'.$portfolio->featured_image)}}" alt="{{__($portfolio->title)}}">
                <div class="project-link text-center">
                  ...
                  @endif
                  <a aria-label="{{__('View Details')}}" href="{{route('front.portfoliodetails', [$portfolio->slug])}}" class="main-btn project-link-btn">{{__('View Details')}}</a>
                </div>
              </div>
            </div>
          @endforeach
