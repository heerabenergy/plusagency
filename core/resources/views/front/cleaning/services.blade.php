                                ...
                                <div class="single-service-item mx-0">
                                    @if (!empty($service->main_image))
                                        <div class="single-service-bg">
                                            <img class="lazy" data-src="{{asset('assets/front/img/services/' . $service->main_image)}}" alt="{{__($service->title)}}">
                                            <span><i class="fas fa-quidditch"></i></span>
                                            @if($service->details_page_status == 1)
                                                <div class="single-service-link">
                                                    <a aria-label="{{__('View More')}}" href="{{route('front.servicedetails', [$service->slug])}}" class="main-btn service-btn">{{__('View More')}}</a>
                                                </div>
                                            @endif
                                        </div>
                ...
                <div class="category-lists job">
                   ...
                   <ul>
                     @foreach ($scats as $key => $scat)
                       <li class="single-category {{$scat->id == request()->input('category') ? 'active' : ''}}"><a aria-label="{{convertUtf8($scat->name)}}" href="{{route('front.services', ['category' => $scat->id, 'term'=>request()->input('term')])}}">{{convertUtf8($scat->name)}}</a></li>
                     @endforeach
                   </ul>
                </div>
             </div>
