                    ...
                    <div class="col-md-6">
                       <div class="single-blog">
                          <div class="blog-img-wrapper">
                             <img aria-label="{{$post->slug}}" src="{{$post->photo}}" alt="{{__($post->slug)}}">
                          </div>
                          <div class="blog-txt">
                            @php
                            ...
                            @endphp
                             ...
                             <h4 class="blog-title"><a aria-label="{{$post->title}}" href="{{route('front.rssdetails', [$post->slug, $post->id])}}">{{convertUtf8(strlen($post->title)) > 40 ? convertUtf8(substr($post->title, 0, 40)) . '...' : convertUtf8($post->title)}}</a></h4>
                    ...
                    <div class="category-lists job">
                       ...
                       <ul>
                          @foreach ($categories as $key => $rcat)
                           <li class="single-category @if($cat_id == $rcat->id) active @endif"><a aria-label="{{$rcat->feed_name}}" href="{{route('front.rcatpost',$rcat->id)}}">{{convertUtf8($rcat->feed_name)}}</a></li>
                          @endforeach
                       </ul>
                    </div>
                 </div>
