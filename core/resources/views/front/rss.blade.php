                    ...
                    <div class="col-md-6">
                       <div class="single-blog">
                          <div class="blog-img-wrapper">
                             <img aria-label="{{$post->slug}}" class="lazy" data-src="{{$post->photo}}" alt="{{__($post->slug)}}">
                          </div>
                          <div class="blog-txt">
                            @php
                            ...
                            @endphp
                             ...
                             <h4 class="blog-title"><a aria-label="{{$post->title}}" href="{{route('front.rssdetails', [$post->slug, $post->id])}}">{{strlen($post->title) > 40 ? mb_substr($post->title, 0, 40, 'utf-8') . '...' : $post->title}}</a></h4>
                             ...
                             <a aria-label="{{__('Read More')}}" href="{{route('front.rssdetails', [$post->slug, $post->id])}}" class="readmore-btn"><span>{{__('Read More')}}</span></a>
              ...
              <div class="sidebar">
                 <div class="blog-sidebar-widgets category-widget">
                    <div class="category-lists job">
                       <h4>{{__('Categories')}}</h4>
                       <ul>
                          <li class="single-category {{empty(request()->input('id')) ? 'active' : ''}}"><a aria-label="{{__('All')}}" href="{{route('front.rss')}}">{{__('All')}}</a></li>
                          @foreach ($categories as $key => $rcat)
                           <li class="single-category {{$rcat->id == request()->input('id') ? 'active' : ''}}"><a aria-label="{{$rcat->feed_name}}" href="{{route('front.rss',['id' => $rcat->id])}}">{{convertUtf8($rcat->feed_name)}}</a></li>
                          @endforeach
                       </ul>
                    </div>
                 </div>
