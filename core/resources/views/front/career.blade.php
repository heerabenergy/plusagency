                  ...
                  @foreach ($jobs as $key => $job)
                    <div class="col-md-12">
                       <div class="single-job @if($loop->last) mb-0 @endif">
                            ...
                            <h3><a aria-label="{{convertUtf8($job->title)}}" href="{{route('front.careerdetails', [$job->slug])}}" class="title">{{convertUtf8($job->title)}}</a></h3>
                ...
                <div class="category-lists job">
                   ...
                   <ul>
                        <li class="single-category {{empty(request()->input('category')) ? 'active' : ''}}">
                            <a aria-label="{{__('All')}}" href="{{route('front.career')}}">{{__('All')}} <span>({{$jobscount}})</span></a>
                        </li>
                        @foreach ($jcats as $key => $jcat)
                            <li class="single-category {{$jcat->id == request()->input('category') ? 'active' : ''}}"><a aria-label="{{convertUtf8($jcat->name)}}" href="{{route('front.career', ['category' => $jcat->id, 'term'=>request()->input('term')])}}">{{convertUtf8($jcat->name)}} <span>({{$jcat->jobs()->count()}})</span></a></li>
                        @endforeach
                   </ul>
                </div>
             </div>
