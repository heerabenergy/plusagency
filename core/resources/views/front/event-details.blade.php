...
@section('breadcrumb-subtitle', strlen($event->title) > 30 ? mb_substr($event->title, 0, 30, 'utf-8') . '...' :
  $event->title)
            ...
            <div class="event-big-slide mb-30">
              @foreach (json_decode($event->image) as $event_image)
                <div class="product-img">
                  <a aria-label="{{__('Event image')}}" href="{{ asset('/assets/front/img/events/sliders/' . $event_image) }}" class="image-popup">
                    <img src="{{ asset('/assets/front/img/events/sliders/' . $event_image) }}" class="img-fluid"
                      alt="{{__('Event image')}}" width="700" height="500">
                  </a>
                </div>
              @endforeach
            </div>
            ...
            <div class="event-content">
              ...
              <div id="purchase-section" style="display: block">
                ...
                <div class="info-box mb-15">
                  <a class="base-color"
                    aria-label="{{$event->eventCategories->name}}" href="{{ route('front.events', ['category' => $event->cat_id]) }}">{{ $event->eventCategories->name }}</a>
                ...
                @if ($event->available_tickets > 0)
                  ...
                  <a aria-label="{{__('Add To Cart')}}" href="javascript:void(0)" class="main-btn" id="addToCart">{{ __('Add To Cart') }}</a>
                @else
              ...
              <div id="invoice-section" style="display: none; justify-content: center; text-align: center">
                @if ($bex->event_guest_checkout == 1 && !Auth::check())
                  <div class="alert alert-warning">
                    ...
                    <a aria-label="{{__('Click Here')}}" href="{{ route('user.login', ['redirected' => 'event']) }}">{{ __('Click Here') }}</a>
                  </div>
                @endif
                <form action="{{ route('front.event.payment') }}" method="POST" enctype="multipart/form-data"
                  ...
                  <div class="form_group"
                    ...
                    <a aria-label="{{__('Cancel')}}" href="javascript:void(0)" class="main-btn" id="cancel"
                      style="height: 45px;justify-content: center;text-align: center; padding-top:10px">{{ __('Cancel') }}</a>
                    <button aria-label="{{__('Confirm')}}" class="main-btn" type="submit"
          ...
          <div class="discription-area mt-80 mb-50">
            <div class="discription-tabs">
              <ul class="nav nav-tabs">
                <li class="nav-item">
                  <a aria-label="{{ __('Details') }}" class="nav-link active" data-toggle="tab" href="#description">{{ __('Details') }}</a>
                </li>
                <li class="nav-item">
                  <a aria-label="{{ __('Organizer') }}" class="nav-link" data-toggle="tab" href="#organizer">{{ __('Organizer') }}</a>
                </li>
                <li class="nav-item">
                  <a aria-label="{{ __('Venue') }}" class="nav-link" data-toggle="tab" href="#vanue">{{ __('Venue') }}</a>
                </li>
              </ul>
            </div>
              ...
              <div id="organizer" class="tab-pane fade">
                <div class="event-content-box">
                  ...
                  @if (!empty($event->organizer_email))
                    <div class="info">
                      <span>{{ __('Email') }}:</span>
                      <p><a aria-label="{{$event->organizer_email}}" href="{{ $event->organizer_email }}">{{ convertUtf8($event->organizer_email) }}</a></p>
                    </div>
                  @endif
                  @if (!empty($event->organizer_website))
                    <div class="info">
                      <span>{{ __('Website') }}:</span>
                      <p><a aria-label="{{$event->organizer_website}}" target="_blank" href="{{ $event->organizer_website }}">{{ convertUtf8($event->organizer_website) }}</a></p>
                    </div>
                  @endif
                </div>
              </div>
              ...
              <div class="event-item">
                <div class="event-img">
                  <img data-src="{{ asset('/assets/front/img/events/sliders/' . json_decode($moreEvent->image)[0]) }}"
                    class="img-fluid lazy" alt="event image">
                </div>
                <div class="event-content">
                  <h4><a
                      aria-label="{{$moreEvent->title}}" target="_blank" href="{{ route('front.event_details', [$moreEvent->slug]) }}">{{ convertUtf8($moreEvent->title) }}</a>
                  </h4>
                  <div class="post-meta mb-2">
                    @if (!empty($moreEvent->venue_location))
                      <span><a aria-label="{{$moreEvent->venue_location}}" target="_blank" href="#">{{ convertUtf8($moreEvent->venue_location) }}</a></span>
                    @endif
                    <span>
                      <a aria-label="{{$moreEvent->date}}" target="_blank" href="#">{{ date_format(date_create($moreEvent->date), 'd M Y') }}</a>
                    </span>
                  </div>
...
@section('scripts')
  ...
  <script src="{{ asset('assets/front/js/event-checkout-stripe.js') }}"></script>
