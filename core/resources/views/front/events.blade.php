            ...
            <div class="event-search-form pt-50 pb-50">
                <div class="row no-gutters">
                    ...
                    <div class="col-lg-2">
                       ...
                       <div class="col-lg-2">
                           <div class="form_group">
                               <button aria-label="{{__('Search')}}" class="main-btn" type="submit" style="height: 45px;justify-content: center;text-align: center;padding-top: 0;padding-bottom: 0;">
                                   {{__('Search')}}</button>
                           </div>
                       </div>
                        ...
                        <div class="col-lg-6 col-md-12 col-sm-12 grid-column cat1 cat4">
                            <div class="event-item">
                                <div class="event-img">
                                    ...
                                    @endphp
                                    <img data-src="{{(!empty($images)) ? asset('/assets/front/img/events/sliders/'.$images[0]) : ''}}" class="img-fluid lazy" alt="event slider image">
                                    <div class="event-overlay">
                                        <a aria-label="{{__('BUY TICKET')}}" href="{{route('front.event_details',[$event->slug])}}" class="main-btn">{{__('BUY TICKET')}}</a>
                                    </div>
                                </div>
                                <div class="event-content">
                                    <a aria-label="{{$event->eventCategories->name}}"   class="cat c-1 base-bg text-white">{{$event->eventCategories->name}}</a>
                                    <a aria-label="{{$event->cost}}" target="_blank" class="cat c-1" href="{{route('front.events', ['category' => $event->eventCategories->id])}}">
                                    ...
                                    <h3><a aria-label="{{$event->title}}" target="_blank" href="{{route('front.event_details',[$event->slug])}}">
