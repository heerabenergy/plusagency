                    ...
                    @auth
                      @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                        @if ($activeSub->first()->current_package_id == $package->id)
                          <a aria-label="{{__('Extend')}}" href="{{route('front.packageorder.index',$package->id)}}" class="finlance_btn">{{__('Extend')}}</a>
                        @else
                          <a aria-label="{{__('Change')}}" href="{{route('front.packageorder.index',$package->id)}}" class="finlance_btn">{{__('Change')}}</a>
                        @endif
                      @elseif ($activeSub->count() == 0)
                        <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="finlance_btn">{{__('Purchase')}}</a>
                      @endif
                    @endauth
                    @guest
                      <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="finlance_btn">{{__('Purchase')}}</a>
                  ...
                  @else
                    @if ($package->order_status != 0)
                      ...
                      <div class="pricing_button">
                        <a aria-label="{{__('Place Order')}}" href="{{ $link }}" @if($package->order_status == 2) target="_blank" @endif class="finlance_btn">{{__('Place Order')}}</a>
                      </div>
                    @endif
                  @endif
                </div>
