                                            ...
                                            <div class="service_overlay">
                                                @if ($service->details_page_status == 1)
                                                    <div class="button_box">
                                                        <a aria-label="{{__($service->title)}}" href="{{route('front.servicedetails', [$service->slug])}}" class="more_icon"><i class="fas fa-angle-double-right"></i></a>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="finlance_content">
                                            <h4><a aria-label="{{__($service->title)}}" class="text-white" @if($service->details_page_status == 1) href="{{route('front.servicedetails', [$service->slug])}}" @endif>{{convertUtf8(strlen($service->title)) > 20 ? convertUtf8(substr($service->title, 0, 20)) . '...' : convertUtf8($service->title)}}</a></h4>
                                        </div>
                ...
                <div class="category-lists job">
                   ...
                   <ul>
                     @foreach ($scats as $key => $scat)
                       <li class="single-category {{$scat->id == request()->input('category') ? 'active' : ''}}"><a aria-label="{{__($scat->name)}}" href="{{route('front.services', ['category' => $scat->id, 'term'=>request()->input('term')])}}">{{convertUtf8($scat->name)}}</a></li>
                     @endforeach
                   </ul>
                </div>
             </div>
