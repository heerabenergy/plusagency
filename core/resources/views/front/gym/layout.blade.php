...
<html lang="{{app()->getLocale()}}" @if ($rtl == 1) dir="rtl" @endif>
   ...
   <body @if($rtl == 1) dir="rtl" @endif>
    ...
    <!--    announcement banner section end   -->
							...
							<div class="widget_box newsletter_box">
								...
								<form id="footerSubscribeForm" action="{{route('front.subscribe')}}" method="post">
									...
									<div class="form_group">
										...
										<button aria-label="{{__('Subscribe')}}" type="submit" class="finlance_btn py-0">{{__('Subscribe')}}</button>
									</div>
								</form>
							</div>
						</div>
        ...
        @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
            <div id="cartIconWrapper">
                <a aria-label="{{__('View Cart')}}" class="d-block" id="cartIcon" href="{{route('front.cart.index')}}">
    ...
    <!--Scroll-up-->
    <a aria-label="{{__('Back to Top')}}" id="scroll_up" ><i class="fas fa-angle-up"></i></a>
