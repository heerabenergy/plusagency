                ...
                <div class="col-lg-3">
                    ...
                    <div class="widget categories-widget">
                        ...
                        @foreach ($categories as $category)
                            ...
                            <a aria-label="{{$category->name}}" class="nav-link {{ $loop->iteration == 1 ? 'show active' : '' }}" data-toggle="tab" href="{{ '#category' . $category->id }}" role="tab" aria-selected="true">{{ $category->name }}</a>
                            </li>
                        @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane {{ $loop->iteration == 1 ? 'show active' : '' }} fade" id="{{ 'category' . $category->id }}">
                            ...
                            @foreach ($qas as $qa)
                                ...
                                <a aria-label="{{$qa->question}}" class="collapsed card-header" id="heading1" href="#" data-toggle="collapse" data-target="{{ '#collapse' . $qa->id }}" aria-expanded="{{ $loop->iteration == 1 ? 'true' : 'false' }}" aria-controls="{{ 'collapse' . $qa->id }}">
                           ...
                           <div class="accordion" id="accordionExample1">
                              ...
                              <div class="card">
                                 <div class="card-header" id="heading{{$faqs[$i]->id}}">
                                    <h2 class="mb-0">
                                       <button aria-label="{{__('Toggle')}}" class="btn btn-link collapsed btn-block text-left" type="button" data-toggle="collapse" data-target="#collapse{{$faqs[$i]->id}}" aria-expanded="false" aria-controls="collapse{{$faqs[$i]->id}}">
