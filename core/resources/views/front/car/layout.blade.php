...
<body @if ($rtl == 1) dir="rtl" @endif>
          ...
          <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-12">
              <div class="widget_box about_widget">
                <img aria-label="{{__('Footer logo')}}" class="lazy" data-src="{{ asset('assets/front/img/' . $bs->footer_logo) }}" alt="Footer logo">
                <p>
              ...
              <div class="widget_box">
                ...
                <ul class="widget_link">
                  @foreach ($ulinks as $key => $ulink)
                    <li><a aria-label="{{$ulink->name}}" href="{{ $ulink->url }}">{{ convertUtf8($ulink->name) }}</a></li>
                  @endforeach
                </ul>
              </div>
            </div>
              ...
              <div class="widget_box newsletter_box">
                ...
                <form id="footerSubscribeForm" action="{{ route('front.subscribe') }}" method="post">
                  ...
                  <div class="form_group">
                    <input aria-label="{{__('Email')}}" type="email" class="form_control" placeholder="{{ __('Enter Email Address') }}"
                      name="email" required>
                    <p id="erremail" class="text-danger mb-0 err-email"></p>
                    <button aria-label="{{__('Subscribe')}}" class="finlance_btn pt-0" type="submit">{{ __('Subscribe') }}</button>
                  </div>
                </form>
              </div>
            </div>
              ...
              <div class="social_box">
                <ul>
                  @foreach ($socials as $key => $social)
                    <li><a aria-label="{{$social->icon}}"   target="_blank" href="{{ $social->url }}"><i class="{{ $social->icon }}"></i></a></li>
                  @endforeach
                </ul>
              </div>
            </div>
  ...
  <div aria-label="{{__('WhatsApp Chat Button')}}" id="WAButton"></div>


  <!--====== PRELOADER PART START ======-->
  @if ($bex->preloader_status == 1)
    <div aria-label="{{__('Preloader')}}" id="preloader">
      <div class="loader revolve">
        <img aria-label="{{__('Preloader')}}" src="{{ asset('assets/front/img/' . $bex->preloader) }}" alt="Preloader">
      </div>
    </div>
  @endif
  <!--====== PRELOADER PART ENDS ======-->
