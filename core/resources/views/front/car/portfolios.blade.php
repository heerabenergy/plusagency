      ...
      <div class="row">
        ...
        @else
          @foreach ($portfolios as $key => $portfolio)
            ...
            <div class="col-lg-6 mb-4 portfolio-column {{ $categoryName }}">
              <div class="grid_item">
                <div class="grid_inner_item">
                  <div class="finlance_img">
                    <img src="{{asset('assets/front/img/portfolios/featured/'.$portfolio->featured_image)}}" class="img-fluid" alt="{{__($portfolio->title)}}">
                    <div class="project_overlay">
                      <div class="finlance_content">
                        <a aria-label="{{__('More')}}" href="{{route('front.portfoliodetails', [$portfolio->slug])}}" class="more_icon">
                          <i class="fas fa-angle-double-right"></i>
                        </a>

                        <h3><a aria-label="{{$portfolio->title}}" href="{{route('front.portfoliodetails', [$portfolio->slug])}}">{{strlen($portfolio->title) > 25 ? mb_substr($portfolio->title, 0, 25, 'utf-8') . '...' : $portfolio->title}}</a></h3>
