                                ...
                                <div class="grid_item text-center">
                                    <div class="grid_inner_item">
                                        ...
                                        <div class="finlance_content">
                                            ...
                                            <a aria-label="{{__('View Services')}}" href="{{route('front.services', ['category'=>$scategory->id])}}" class="btn_link">{{__('View Services')}}</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="grid_inner_item">
                                        @if (!empty($service->main_image))
                                            <div class="finlance_icon" style="margin-bottom: 20px;">
                                                <img aria-label="{{__('service')}}" class="lazy" data-src="{{asset('assets/front/img/services/' . $service->main_image)}}" alt="service" />
                                            </div>
                                        @endif
                                        <div class="finlance_content">
                                            ...
                                            @if($service->details_page_status == 1)
                                                <a aria-label="{{__('Read More')}}" href="{{route('front.servicedetails', [$service->slug, $service->id])}}" class="btn_link">{{__('Read More')}}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
				...
				<div class="row align-items-center">
					<div class="col-lg-4">
						<div class="play_box text-center">
							<a aria-label="{{__('Play')}}" href="{{$bs->intro_section_video_link}}" class="play_btn"><i class="fas fa-play"></i></a>
						</div>
					</div>
					<div class="col-lg-8">
						<div class="finlance_content_box">
                            ...
                            @if (!empty($bs->intro_section_button_url) && !empty($bs->intro_section_button_text))
                                <div class="button_box">
                                    <a aria-label="{{$bs->intro_section_button_text}}" href="{{$bs->intro_section_button_url}}" class="finlance_btn">{{convertUtf8($bs->intro_section_button_text)}}</a>
                                </div>
                            @endif
						</div>
					</div>
				...
				<div class="testimonial_slide">
                    @foreach ($testimonials as $key => $testimonial)
                        <div class="testimonial_box">
                            <div class="quote">
                                <img aria-label="{{__('Quote')}}" data-src="{{asset('assets/front/img/quote.png')}}" class="lazy" alt="Quote">
                            </div>
                            <div class="client_box">
                                <div class="thumb">
                                    <img aria-label="{{$testimonial->name}}" data-src="{{asset('assets/front/img/testimonials/'.$testimonial->image)}}" class="lazy" alt="{{__($testimonial->name)}}">
                                </div>
                                <div class="info">
                        ...
                        <div class="grid_item">
                            <div class="grid_inner_item">
                                <div class="finlance_img">
                                    <img aria-label="{{$portfolio->title}}" data-src="{{asset('assets/front/img/portfolios/featured/'.$portfolio->featured_image)}}" class="img-fluid lazy" alt="{{__($portfolio->title)}}">
                                    <div class="project_overlay">
                                        <div class="finlance_content">
                                            <a aria-label="{{__('More')}}" href="{{route('front.portfoliodetails', [$portfolio->slug, $portfolio->id])}}" class="more_icon"><i class="fas fa-angle-double-right"></i></a>

                                            <h3><a aria-label="{{$portfolio->title}}" href="{{route('front.portfoliodetails', [$portfolio->slug, $portfolio->id])}}">{{strlen($portfolio->title) > 25 ? mb_substr($portfolio->title,0,25,'utf-8') . '...' : $portfolio->title}}</a></h3>
					...
					<div class="col-lg-6">
						<div class="finlance_content_box">
                            ...
                            @if (!empty($bs->approach_button_url) && !empty($bs->approach_button_text))
                                <div class="button_box">
                                    <a aria-label="{{$bs->approach_button_text}}" href="{{$bs->approach_button_url}}" class="finlance_btn">{{convertUtf8($bs->approach_button_text)}}</a>
                                </div>
                            @endif
						</div>
					</div>
                                    ...
                                    <div class="team_overlay">
                                        <ul class="social_box">
                                            @if (!empty($member->facebook))
                                                <li><a aria-label="{{__('Facebook')}}" href="{{$member->facebook}}" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                                            @endif
                                            @if (!empty($member->twitter))
                                                <li><a aria-label="{{__('Twitter')}}" href="{{$member->twitter}}" target="_blank"><i class="fab fa-twitter"></i></a></li>
                                            @endif
                                            @if (!empty($member->linkedin))
                                                <li><a aria-label="{{__('Linkedin')}}" href="{{$member->linkedin}}" target="_blank"><i class="fab fa-linkedin-in"></i></a></li>
                                            @endif
                                            @if (!empty($member->instagram))
                                                <li><a aria-label="{{__('Instagram')}}" href="{{$member->instagram}}" target="_blank"><i class="fab fa-instagram"></i></a></li>
                                            @endif
                                        </ul>
                                    </div>
                                </div>
			...
			<div class="container">
				<div class="partner_slide">
					...
					<div class="single_partner">
						<a aria-label="{{__('Partner Logo')}}" href="{{$partner->url}}" target="_blank"><img data-src="{{asset('assets/front/img/partners/'.$partner->image)}}" class="img-fluid lazy" alt="Partner Logo"></a>
                    </div>
                    @endforeach
				</div>
			</div>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="finlance_content">
                                            ...
                                            <div class="post_meta">
                                                ...
                                                <span><i class="far fa-calendar-alt"></i><a aria-label="{{$blogDate}}"  href="#">{{$blogDate}}</a></span>
                                            </div>
                                            <h3 class="post_title"><a aria-label="{{$blog->title}}" href="{{route('front.blogdetails', [$blog->slug, $blog->id])}}">{{strlen($blog->title) > 40 ? mb_substr($blog->title,0,40,'utf-8') . '...' : $blog->title}}</a></h3>
                                            <p>{!! strlen(strip_tags($blog->content)) > 100 ? mb_substr(strip_tags($blog->content),0,100,'utf-8') . '...' : strip_tags($blog->content) !!}</p>
                                            <a aria-label="{{__('Read More')}}" href="{{route('front.blogdetails', [$blog->slug, $blog->id])}}" class="finlance_btn">{{__('Read More')}}</a>
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="finlance_img">
                                            <a aria-label="{{__('Blog Image')}}" href="{{route('front.blogdetails', [$blog->slug, $blog->id])}}"><img data-src="{{asset('assets/front/img/blogs/'.$blog->main_image)}}" class="img-fluid lazy" alt="Blog Image"></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
		...
		<section class="finlance_cta cta_v1 pt-70 pb-70" style="background-image: url({{asset('assets/front/img/pattern_bg_2.jpg')}});">
			<div class="container">
				<div class="row align-items-center">
					...
					<div class="col-lg-4">
						<div class="button_box">
							<a aria-label="{{$bs->cta_section_button_text}}" href="{{$bs->cta_section_button_url}}" class="finlance_btn">{{convertUtf8($bs->cta_section_button_text)}}</a>
						</div>
					</div>
				</div>
			</div>
