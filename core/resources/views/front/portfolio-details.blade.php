    ...
    <div class="row">
      <div class="col-lg-7 col-xl-7">
        <div class="project-ss-carousel owl-carousel owl-theme common-carousel">
          ...
          <a aria-label="{{$pi->image}}" href="#" class="single-ss" data-id="{{$pi->id}}">
            <img class="lazy" data-src="{{asset('assets/front/img/portfolios/sliders/'.$pi->image)}}" alt="portfolio slider image">
          </a>
          @endforeach
        </div>
        @foreach ($portfolio->portfolio_images as $key => $pi)
        <a aria-label="{{$pi->image}}" id="singleMagnificSs{{$pi->id}}" class="single-magnific-ss d-none"
            ...
            <div class="col-xl-12 col-lg-12 col-md-12">
              <div class="project-infos">
                ...
                @if ($portfolio->website_link != null)
                  <div class="row mb-0">
                    <div class="col-12">
                        <a aria-label="{{__('Live Demo')}}" target="_blank" href="{{$portfolio->website_link}}" class="btn base-bg text-white btn-sm">{{__('Live Demo')}}</a>
                    </div>
                  </div>
                @endif
              </div>
