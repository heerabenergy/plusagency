                                                    ...
                                                    @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                                                        <td>
                                                            @if ($activeSub->first()->current_package_id == $package->id)
                                                                <a aria-label="{{__('Extend')}}" href="{{route('front.packageorder.index',$package->id)}}" class="btn base-bg text-white">{{__('Extend')}}</a>
                                                            @else
                                                                <a aria-label="{{__('Change')}}" href="{{route('front.packageorder.index',$package->id)}}" class="btn base-bg text-white">{{__('Change')}}</a>
                                                            @endif
                                                        </td>
                                                    @elseif ($activeSub->count() == 0)
                                                        <td>
                                                            <a aria-label="{{__('Purchase')}}" href="{{route('front.packageorder.index',$package->id)}}" class="btn base-bg text-white">{{__('Purchase')}}</a>
                                                        </td>
                                                    @endif
                                                </tr>
                                                @endforeach
