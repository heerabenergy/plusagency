    ...
    <ul class="links">
      <li>
        ...
        >{{__('Dashboard')}}</a>
            ...
            <li><a aria-label="{{__('Packages')}}" class="@if(request()->path() == 'user/packages') active @endif" href="{{route('user-packages')}}">{{__('Packages')}}</a></li>
            @endif
      ...
      @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
        <li>
          <a
          aria-label="{{__('Shipping Details')}}"
      ...
      <li><a aria-label="{{__('Logout')}}" href="{{route('user-logout')}}">{{__('Logout')}}</a></li>
    </ul>
  </div>
</div>
